...
library(httr)
r <- GET(base_url, query=list(q="human rights","api-key"=apikey))
r
```
...
```{r}
r <- GET(base_url, query=list(q="international criminal court",
                              "api-key"=apikey,
                              "begin_date"=20140101,
                              "end_date"=20141231))
json <- content(r, 'parsed')
...
Between these two dates, there were 8,780 articles in the NYT mentioning "human rights".
...
nyt_count(q="international criminal court", date1=20160101, date2=20160122)
```
...
nyt_years_count(q="international criminal court", yearinit=1950, yearend=2014)
```
...
```{r}
counts <- nyt_years_count(q="european court of human rights", yearinit=1950, yearend=2014)
plot(1950:2014, counts, type="l", main="Mentions of ECHR on the NYT, by year",
...
```{r}
counts <- nyt_dates_count(q="karadzic", init = as.Date("2014/01/01"), end = as.Date("2016/03/30"), by="month")
